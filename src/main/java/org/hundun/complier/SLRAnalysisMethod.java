 ...
 * LR0分析算法
 */
public class SLRAnalysisMethod {
    //非终结符对应公式表
    ...
    //根据ref定制的集合方便查询与去重
    static class LinkedHashSetD extends LinkedHashSet<SLRAnalysisMethod.Ref> {
        @Override
...
//                "T>y",
        ...
        //SLR解决归约与递进冲突 引入LL1中的follow集在归约时不在盲目设置归约而是只设定follow集中的字符进行归约
        LL1AnalysisMethod.init(data);
                ...
                if (D.size() == 1) {
                    ...
                    if (characters[characters.length - 1] == POINT) {
                        ...
                        if (mapIndex1.index == 0) {
                            ...
                            //根据follow集进行配置归约
                            Set<String> follow = LL1AnalysisMethod.follow.get(mapIndex1.data);
                            for (String s2 : follow) {
                                ACTION[ref1.index][s2.charAt(0)] = 'R' << 8 | mapIndex1.index;
                                //否则其他填充数据位Rx,x代表归约公式
                            }
