package org.hundun.complier.lr1.lr1;


import org.hundun.complier.lr1.util.Rule;

import java.util.Arrays;
import java.util.Objects;

public class LR1ItemEquals extends Rule {

    protected int dotPointer;

    public LR1ItemEquals(Rule r) {
        super(r.getLeftSide(), r.getRightSide());
        int finished = 0;
        if (r.getRightSide().length == 1 && r.getRightSide()[0].equals("epsilon")) {
            finished = 1;
        }
        this.dotPointer = finished;
    }

    public LR1ItemEquals(String leftSide, String[] rightSide, int dotPointer) {
        super(leftSide, rightSide);
        this.dotPointer = dotPointer;
    }

    public LR1ItemEquals(LR1ItemEquals item) {
        super(item);
        dotPointer = item.getDotPointer();
    }

    @Override
    public int hashCode() {
        int hash = 7;
        hash = 89 * hash + this.dotPointer;
        hash = 89 * hash + Objects.hashCode(this.leftSide);
        hash = 89 * hash + Arrays.deepHashCode(this.rightSide);
        return hash;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (obj == null) {
            return false;
        }
        if (getClass() != obj.getClass()) {
            return false;
        }
        final LR1ItemEquals other = (LR1ItemEquals) obj;
        if (this.dotPointer != other.dotPointer) {
            return false;
        }
        if (!this.leftSide.equals(other.leftSide)) {
            return false;
        }
        if (!Arrays.equals(this.rightSide, other.rightSide)) {
            return false;
        }
        return true;
    }

    @Override
    public String toString() {
        String str = leftSide + " -> ";
        for (int i = 0; i < rightSide.length; i++) {
            if (i == dotPointer) {
                str += ".";
            }
            str += rightSide[i];
            if(i != rightSide.length - 1){
                str+= " ";
            }
        }
        if (rightSide.length == dotPointer) {
                str += ".";
        }
        return str;
    }
    

    public int getDotPointer() {
        return dotPointer;
    }

    boolean goTo() {
        if (dotPointer >= rightSide.length) {
            return false;
        }
        dotPointer++;
        return true;
    }

    String getCurrentTerminal() {
        if(dotPointer == rightSide.length){
            return null;
        }
        return rightSide[dotPointer];
    }

}
